...
export default function VistaCircuitos({ mostrarBotonReserva = false, soloIncompletos = false }) {
  const [circuitos, setCircuitos] = useState([]);
  const [filtroLocalidad, setFiltroLocalidad] = useState('');
  const [filtroDistancia, setFiltroDistancia] = useState([0, 10]);
  ...
  const filtrados = circuitos.filter(c =>
    (!soloIncompletos || c.cupo_lleno || !c.disponible_para_inscripcion) &&
    (!filtroLocalidad || c.Localidad === filtroLocalidad) &&
    (c.Distancia >= filtroDistancia[0] * 1000 && c.Distancia <= filtroDistancia[1] * 1000) &&
    (filtroDias.length === 0 || filtroDias.some(d => c.Dias.includes(d)))
  );

  return (
    <section className="py-16 px-6 bg-white max-w-screen-xl mx-auto">
      <h2 className="text-4xl font-bold text-center text-[#0F172A] mb-4">🧭 Circuitos disponibles</h2>
      <p className="text-center text-gray-600 mb-10 text-base">
        Explorá nuestros circuitos activos, con cupos actualizados.
      </p>
